...
router.post('/users/forgetPassword', async (request, response) => {
    ...
    try {
        ...
        await otp.save()
        const data = {
            host_id: process.env.EMAIL_HOST_ID,
            name: 'Omnimemor',
            to: [
                request.body.email
            ],
            subject: `Hello ${user.name} | OTP 👨‍💻 | noreply`,
            html_body: `<b>Hello ${user.name}</b><br><br>Your OTP is <b>${otp.otp}</b>. <i>Be quick, it will be expired on ${otp.expiryDateTime.toLocaleString()}</i>.<br>Thank You! <br><br><b>Best</b> <br><i>Server Admin</i>`
        }
        sendMail(data)
...
router.post('/users/verifyOtp', async (request, response) => {
    try {
...
router.post('/users/resetPassword', passwordResetAuth, async (request, response) => {
    try {
        const currentTime = new Date()
